...
* """
* Player1+ +8/23/2021_6:34:56_PM,-8/23/2021_6:35:04_PM,+8/23/2021_6:37:47_PM,-8/23/2021_6:37:54 PM
...
namespace MCGalaxy
{
    public class TimeManagement : Plugin
    {
        ...
        private void DeleteOldTMData()
        {
            foreach (string pname in TMTable.AllNames())
            {
                ...
                while (sinceOldestLogout.TotalSeconds > 60 && timeStamps.Count >= 2)
                {
        ...
        private int GetTotalTimeSpent(string pname)
        {
            string data = TMTable.FindData(pname);
            ...
            // Edge case - Last entry is a login event. Pretend there's a logout event at the end that's the current date
        ...
        private bool IsValidTimestamps(List<string> timeStamps)
        {
            int stampCount = timeStamps.Count;
            // Return true if it's just a single login event. Return false if we start with a logout event
            if (stampCount == 1 && timeStamps[0][0] == '+')
            {
                return true;
            }
            ...
            // Return false if stamps[i] == stamps[i + 1] (e.g. login followed by login)
        ...
        private List<string> IterativelyCleanTimeStamps(List<string> stamps, string pName)
        {
        ...
        private void MessageOnlinePlayers()
        {
            ...
            {
                int timeSpent = GetTotalTimeSpent(p.name);
    ...
    public sealed class CmdTimeManagement : Command2
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            if (args.Length != 1 || (args[0] != "daily" && args[0] != "current")) { Help(p); return; }  // Force people to write "/tm current" OR "/tm daily"
        ...
        {
            p.Message("%T/TimeManagement [daily/current]%H- Toggles time management mode");
